...
/usr/bin/time -v kmtricks pipeline --file "$fof_salmonella_150k" --run-dir "$index_dir"/kmindex/index_salmonella_150k_on_disk --kmer-size 25 --hard-min 1 --mode hash:bf:bin --bloom-size 36754250 -t 32 --until count >> "$log_prefix/kmindex/$log_suffix" 2>&1
